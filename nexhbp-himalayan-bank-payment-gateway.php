...
function hbl_himalayan_bank_payment_gateway_init(): void {
	...
	load_plugin_textdomain( 'hbl-himalayan-bank-payment-gateway',
		false,
	...
	class HBL_Himalayan_Bank_Payment_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->method_description = esc_html__( 'Himalayan Bank payment gateway integration for WooCommerce.',
		...
		public function init_form_fields(): void {
			$this->form_fields = array(
				...
				'title'                        => array(
					...
					'type'        => 'text',
					'description' => esc_html__( 'This controls the title that the user sees during checkout.',
						'hbl-himalayan-bank-payment-gateway' ),
				...
				'currency'                     => array(
					...
					'type'        => 'text',
					'description' => esc_html__( 'Enter your currency. Example: USD, EUR, etc. Please contact Himalayan Bank for supported currencies.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => 'USD',
					'desc_tip'    => false,
				),
				'3d_secure'                    => array(
					...
					'description' => esc_html__( 'Enable 3D Secure for this payment gateway. Turn it off for test mode.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => 'yes',
					'desc_tip'    => false,
				),
				'enabled_test_mode'            => array(
					...
					'description' => esc_html__( 'Turn on to enable test mode for this payment gateway.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => 'yes',
					'desc_tip'    => false,
				),
				...
				'merchant_sign_private_key'    => array(
					...
					'type'        => 'textarea',
					'description' => esc_html__( 'Enter your Merchant Signing Private Key.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => '',
					'desc_tip'    => false,
				),
				'merchant_decrypt_private_key' => array(
					...
					'type'        => 'textarea',
					'description' => esc_html__( 'Enter your Merchant Encryption Private Key.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => '',
					'desc_tip'    => false,
				),
				...
				'paco_encrypt_public_key'      => array(
					...
					'type'        => 'textarea',
					'description' => esc_html__( 'Enter your PACO Encryption Public Key.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => '',
					'desc_tip'    => false,
				),
				...
				'failure_message'              => array(
					...
					'type'        => 'textarea',
					'description' => esc_html__( 'Message displayed on payment failure.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => esc_html__( 'Oops! Something went wrong with your payment.',
						'hbl-himalayan-bank-payment-gateway' ),
					'desc_tip'    => false,
				),
				'cancel_message'               => array(
					...
					'type'        => 'textarea',
					'description' => esc_html__( 'Message displayed when payment is cancelled.',
						'hbl-himalayan-bank-payment-gateway' ),
					'default'     => esc_html__( 'Payment cancelled. If you have any questions, please contact us.',
						'hbl-himalayan-bank-payment-gateway' ),
					'desc_tip'    => false,
				),
	...
	function nexhbp_add_himalaya_bank_payment_gateway( $methods ) {
		$methods[] = 'HBL_Himalayan_Bank_Payment_Gateway';

		return $methods;
	}

	add_filter( 'woocommerce_payment_gateways', 'nexhbp_add_himalaya_bank_payment_gateway' );

}

add_action( 'template_redirect', 'nexhbp_himalayan_bank_payment_gateway_handle_payment_redirect' );
/**
 * @throws WC_Data_Exception
 */
function nexhbp_himalayan_bank_payment_gateway_handle_payment_redirect(): void {
	global $post;
	...
	if ( $action && $order_id && $payment_nonce && is_checkout() && ! wp_verify_nonce( $payment_nonce, 'payment_action' ) ) {
		wc_add_notice( esc_html__( 'Security check failed, please try again.', 'nexhbp-himalayan-bank-payment-gateway' ), 'error' );
	...
	$success_page_id = get_option( 'woocommerce_nexhbp_himalayan_bank_payment_gateway_settings' )['success_page'];
	...
	if ( ! empty( $action ) && is_checkout() ) {
		...
		$settings = get_option( 'woocommerce_nexhbp_himalayan_bank_payment_gateway_settings' );

		$failure_message = $settings['failure_message'] ? esc_html( $settings['failure_message'] ) : esc_html__( 'Payment failed. Please try again.', 'nexhbp-himalayan-bank-payment-gateway' );
		$cancel_message  = $settings['cancel_message'] ? esc_html( $settings['cancel_message'] ) : esc_html__( 'Payment was cancelled. If this was a mistake, please try again.',
			'nexhbp-himalayan-bank-payment-gateway' );
