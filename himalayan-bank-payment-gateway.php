...
function hbl_woocommerce_missing_notice(): void
{
    if (!class_exists("WooCommerce")) {
        $message = sprintf(
            ...
            esc_html__(
                "Himalayan Bank Payment Gateway requires WooCommerce to be installed and active. You can download %s here.",
...
function hbl_add_settings_link($links)
{
    $settings_link =
        '<a href="' .
        admin_url("admin.php?page=wc-settings&tab=checkout&section=hbl_himalayan_bank_payment_gateway") .
        '">' .
        esc_html__("Settings", "hbl-himalayan-bank-payment-gateway") .
        "</a>";
...
function hbl_himalayan_bank_payment_gateway_init(): void
{
    ...
    load_plugin_textdomain("hbl-himalayan-bank-payment-gateway", false, basename(__DIR__) . "/languages");
    ...
    class HBL_Himalayan_Bank_Payment_Gateway extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            add_action("woocommerce_review_order_before_submit", [$this, "display_card_fee_message"]);
            add_action("woocommerce_api_hbl_himalayan_bank_payment_gateway", [$this, "check_ipn_response"]);
        }
        ...
        public function get_icon(): string
        {
            ...
            return '<img src="' .
                esc_url($icon_url) .
                '" alt="Himalayan Bank" title="Himalayan Bank" style="max-height: 32px;" />';
        }
        ...
        {
            if (
                $this->get_option("card_fee_enabled") === "yes" &&
                $this->get_option("card_fee_percentage") > 0
            ) {
                $message = $this->get_option("card_fee_message");
                if (empty($message)) {
                    $message = sprintf(
                        esc_html__(
                            "Note: You will be charged an extra %s%% bank charges when using %s.",
                            "hbl-himalayan-bank-payment-gateway",
                        ),
        ...
        public function init_form_fields(): void
        {
            $this->form_fields = [
                ...
                "paco_sign_public_key" => [
                    ...
                    "type" => "textarea",
                    "description" => esc_html__(
                        "Enter your PACO Signing Public Key.",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                    "default" => "",
                    "desc_tip" => false,
                ...
                "card_fee_percentage" => [
                    ...
                    "type" => "number",
                    "description" => esc_html__(
                        "Enter the card fee percentage to be added to the total amount",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                    "default" => "0",
                    "custom_attributes" => [
                ...
                "card_fee_message" => [
                    ...
                    "type" => "textarea",
                    "description" => esc_html__(
                        "Message to display about the card fee on checkout. Leave blank for default message.",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                    "default" => "",
                    "desc_tip" => false,
                ...
                "redirect_pages" => [
                    ...
                    "type" => "title",
                    "description" => esc_html__(
                        "Set custom pages for payment responses. Leave blank to use default WooCommerce pages.",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                ],
                "success_page" => [
                    ...
                    "type" => "select",
                    "description" => esc_html__(
                        "Page to redirect to after successful payment",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                    "default" => "",
                ...
                "failure_page" => [
                    ...
                    "type" => "select",
                    "description" => esc_html__(
                        "Page to redirect to after failed payment",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                    "default" => "",
                    "options" => $this->get_pages_list(),
                ...
                "cancel_page" => [
                    ...
                    "type" => "select",
                    "description" => esc_html__(
                        "Page to redirect to after cancelled payment",
                        "hbl-himalayan-bank-payment-gateway",
                    ),
                    "default" => "",
                    "options" => $this->get_pages_list(),
        ...
        public function process_payment($order_id): array
        {
            ...
            if ($order->has_status("pending") && !empty($existing_url) && !empty($expiry_datetime)) {
                ...
                if ($expiry > $now) {
                    ...
                    $context = ["source" => "hbl-himalayan-bank-payment-gateway"];
                    $logger?->info(
                        "Existing payment URL has expired. Generating a new payment URL.",
                        $context,
                    );
        ...
        {
            $logger = wc_get_logger();
            $context = ["source" => "hbl-himalayan-bank-payment-gateway-ipn"];
            ...
            try {
                ...
                $payment = new Payment();
                $decrypting_key = $payment->GetPrivateKey(
                    SecurityData::get_merchant_decryption_private_key(),
                );
                $signature_verification_key = $payment->GetPublicKey(
                    SecurityData::get_paco_signing_public_key(),
                );
                $decrypted_data = $payment->DecryptToken(
                    $raw_data,
                    $decrypting_key,
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    $logger->error(
                        "Invalid JSON in IPN after decryption: " . json_last_error_msg(),
                        $context,
                    );
                    $this->respond_to_ipn(400);
                    return;
        ...
        private function process_payment_status($order, $payment_status, $card_info, $logger, $context): void
        {
            switch ($payment_status) {
                case "PCPS":
                    $order->update_status(
                        "pending",
                        "Payment pre-stage: awaiting payer to complete payment.",
                    );
                    break;
                ...
                case "A":
                    if (!$order->has_status("completed")) {
                        ...
                        $order->add_order_note(
                            "Payment approved via Himalayan Bank Gateway IPN." . $card_info,
                        );
                ...
                case "C":
                    ...
                    $logger->info(
                        "Payment cancelled/voided for order {$order->get_id()}: $payment_status",
                        $context,
                    );
                    break;
                ...
                case "S":
                    if (!$order->has_status("completed")) {
                        $order->payment_complete();
                        $order->add_order_note(
                            "Payment settled via Himalayan Bank Gateway IPN." . $card_info,
                        );
                ...
                case "P":
                    $order->update_status(
                        "on-hold",
                        "Payment pending approval via Himalayan Bank Gateway IPN.",
                    );
                    break;
...
function hbl_himalayan_bank_payment_gateway_handle_payment_redirect(): void
{
    ...
    // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
    $transaction_id = isset($_GET["controllerInternalId"])
        ? sanitize_text_field(wc_clean(wp_unslash($_GET["controllerInternalId"])))
        : null;
    // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
    $action = isset($_GET["action"]) ? sanitize_text_field(wc_clean(wp_unslash($_GET["action"]))) : "";
    // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
    $payment_nonce = isset($_GET["payment_nonce"])
        ? sanitize_text_field(wc_clean(wp_unslash($_GET["payment_nonce"])))
        : null;
    // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
    ...
    if (
        ...
        $payment_nonce &&
        is_checkout() &&
        !wp_verify_nonce($payment_nonce, "payment_action")
    ) {
        wc_add_notice(
            esc_html__("Security check failed, please try again.", "hbl-himalayan-bank-payment-gateway"),
            "error",
        );
    ...
    if ($order_id && $transaction_id && is_order_received_page()) {
        ...
        if ($order && $provided_salt) {
            ...
            if (hbl_verify_payment_salt($order_id, $provided_salt)) {
                ...
                $logger->error(
                    "Invalid salt provided for order {$order_id}. Potential security breach.",
                    $context,
                );
                wc_add_notice(
                    esc_html__(
    ...
    if ($order_id && $success_page_id !== "" && $post && $post->ID === (int) $success_page_id) {
        ...
        if ($order && $provided_salt) {
            ...
            if (hbl_verify_payment_salt($order_id, $provided_salt)) {
                $order->update_status(
                    "processing",
                    "Payment confirmed via custom success page with valid security token",
                );
                if ($transaction_id) {
                    $order->set_transaction_id($transaction_id);
                }
                ...
                $order->save();
                $logger->info(
                    "Payment confirmed for order {$order_id} via custom success page with valid salt",
                    $context,
                );
            } else {
                $logger->error(
                    "Invalid salt provided for order {$order_id} on custom success page. Potential security breach.",
                    $context,
                );
                wc_add_notice(
                    esc_html__(
    ...
    if (!empty($action) && is_checkout()) {
        ...
        $failure_message = $settings["failure_message"]
            ? esc_html($settings["failure_message"])
            : esc_html__("Payment failed. Please try again.", "hbl-himalayan-bank-payment-gateway");
        $cancel_message = $settings["cancel_message"]
            ? esc_html($settings["cancel_message"])
            : esc_html__(
